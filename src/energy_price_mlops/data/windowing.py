...
class PriceWindowDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    ...
    def __init__(
        ...
        *,
        ...
        self.values = normalizer.transform_array(values)
        ...
        if last_index + horizon_size > len(values):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        inputs = torch.from_numpy(self.values[context_start:forecast_start])
        targets = torch.from_numpy(self.values[forecast_start:forecast_end])
